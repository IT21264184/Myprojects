...
document.querySelector('#cart').onclick = () => {
	$('.modalc').removeClass("hide");
	$('.modalc').addClass("show");
	$('.modalc').addClass("show-cart");
...
function getCart(pageLink) {
	...
	$.get("http://localhost:8080/Online_food_ordering_system/GetCartServlet", { pageLink: pageLink }, function(response) {
...
function buildCart(cartItems) {
	...
	if (!callCart) {
		for (var i = 0; i < cartItems.length; i++) {
			...
			var options = createOptions(qty);

			var itemList = `
							<li class="flexbody" id="list">
							<div class="cartitem">
				              <select onChange="changeQty(${cartItems[i].item_id}, this)" class="custom-select">
								${options}
				              </select>
				
				
				              <h4>${cartItems[i].item_name}<h4>
				            </div>
				            <div class="cart-delete">
				              <img src="${cartItems[i].img_path}" alt="">
				              <i class="fa-solid fa-trash" onclick="clearItem(${cartItems[i].item_id})"></i>
				            </div>
				            </li>`
...
function clearCart(pageLink){
	$.post("http://localhost:8080/Online_food_ordering_system/ClearCartServlet", {pageLink : pageLink})
}


function clearItem(item_id){
	$.post("http://localhost:8080/Online_food_ordering_system/ClearItemByIdServlet", {item_id : item_id})
}
...
function changeQty(item_id, qty){
	...
	$.post("http://localhost:8080/Online_food_ordering_system/ChangeQuantityServlet", {item_id : item_id, quantity : quantity})
}

function addtoCart(item_id, quantity, notes){
	$.post("http://localhost:8080/Online_food_ordering_system/AddToCartServlet", {item_id : item_id, quantity : quantity, notes : notes}, function(response){
	    ...
	    msg.innerHTML = `<span class="fa-solid fa-cart-plus"></span> <span class="favmsg">${response}</span>`
	})
}
...
function test(item_id){
	...
	var notes = document.getElementById('review').value
	
	$.post("http://localhost:8080/Online_food_ordering_system/AddToCartServlet", {item_id : item_id, notes : notes, quantity : quantity})
}


function addToFavourites(item_id){
	$.post("http://localhost:8080/Online_food_ordering_system/AddToFavouriteServlet", {item_id : item_id}, function(response){
    ...
    document.getElementById('overlay').style.opacity = "1";
		var msg = document.getElementById('respMsg')
		msg.innerHTML = `<span class="fa-solid fa-heart"></span> <span class="favmsg"> ${response} </span>`
